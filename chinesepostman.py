...
class ChinesePostman(object):
    ...
    def run(self):
        ...
        #Validation for selected features
        if layer is None:
        ...
        if len(features) == 0:
            QMessageBox.information(None, "Chinese Postman", "Please select an area. The 'Select Features by Polygon' tool " +
                                                            "works well for this.")
            return
        #convert features in NetworkX graph
        graph = build_graph(features)
        #take largest connected network in case multiple are selected
        ...
        #Output a list of node coords (to possible make a better route viewer somewhere later)
        nodes = [list(ele) for ele in nodes]
        for x in nodes:
            x[0], x[1] = x[1], x[0]
        nodes = nodes[0::10]
        nodes = [tuple(ele) for ele in nodes]
        info = ""
        info += "Every 10th Node Coordinates: "
        info += str(nodes)
        QMessageBox.information(None, "node coords", info)
